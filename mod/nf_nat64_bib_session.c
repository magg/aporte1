...
void nat64_session_renew(struct nat64_st_entry *session, enum expiry_type type)
{
	...
	pr_info("NAT64: [session] Renewing session %pI4:%hu (timeout %u sec).\n",
...
void nat64_clean_expired_sessions(struct list_head *queue, int expiry_type)
{
	...
	list_for_each_safe(pos, n, queue) {
		...
		if (time_after(jiffies, session->expires)) {
			...
			pr_info("NAT64: [garbage-collector] removing session %pI4:%hu\n",
			        &session->remote4_addr, ntohs(session->remote4_port));
			list_del(pos);
        ...
        enum expiry_type type)
{
	...
	pr_info("NAT64: [session] New session (timeout %u sec).\n",
	        expiry_base[type].timeout);
	pr_info("NAT64: [session] x:%hu \tX':%pI6c.\n", ntohs(s->remote6_port),
	        &s->remote6_addr);
	pr_info("NAT64: [session] y:%hu \tY':%pI6c.\n", ntohs(s->embedded6_port),
	        &s->embedded6_addr);
	pr_info("NAT64: [session] t:%hu \tT:%pI4.\n", ntohs(s->local4_port),
	        &s->local4_addr);
	pr_info("NAT64: [session] z:%hu \tZ(Y'):%pI4.\n", ntohs(s->remote4_port),
	        &s->remote4_addr);
        ...
        enum expiry_type type)
{
	...
	pr_info("NAT64: [session] New session (timeout %u sec).",
	        expiry_base[type].timeout);
	pr_info("NAT64: [session] x:%hu\tX':%pI6c.", ntohs(s->remote6_port),
	        &s->remote6_addr);
	pr_info("NAT64: [session] y:%hu\tY':%pI6c.", ntohs(s->embedded6_port),
	        &s->embedded6_addr);
	pr_info("NAT64: [session] t:%hu\tT':%pI4.", ntohs(s->local4_port),
	        &s->local4_addr);
	pr_info("NAT64: [session] z:%hu\tZ':%pI4.", ntohs(s->remote4_port),
	        &s->remote4_addr);
