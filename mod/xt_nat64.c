...
unsigned int nat64_core(struct sk_buff *skb, 
    ...
    if (!new_skb) {
		...
		new_skb->dev = nat64_dev;
		nat64_dev->stats.rx_packets++;

pr_debug("NAT64: skb len %d", new_skb->len);
		nat64_dev->stats.rx_bytes += new_skb->len;
		netif_rx(new_skb);
...
unsigned int nat64_tg4(struct sk_buff *skb)
{
    ...
    if (l4_protocol & NAT64_IP_ALLWD_PROTOS) {
         ...
         */
        return nat64_core(skb, NFPROTO_IPV4, l4_protocol);
    }
...
unsigned int nat64_tg6(struct sk_buff *skb)
{
   ...
   // if (!nat64_tg6_cmp(&info->ip6dst.in6, &info->ip6dst_mask.in6, &iph->daddr, info->flags))
	...
	if(memcmp(&iph->daddr, &prefix_base, ipv6_pref_len / 8))	
        	return NF_ACCEPT;
    ...
    if (l4_protocol & NAT64_IPV6_ALLWD_PROTOS) {
         ...
         * Core functions of the NAT64 implementation.
         */
        return nat64_core(skb, NFPROTO_IPV6, l4_protocol);
    }
...
int __init nat64_init(void)
{
	...
	ret = in6_pton(ipv6_pref_addr_str, -1, (u8 *)&prefix_base, '\0', NULL);
	if (!ret)
	{
