...
static bool nat64_get_skb_from6to6(struct sk_buff * old_skb,
	...
	pr_debug("NAT64: paylen %hu", htons(pay_len));
...
static bool nat64_get_skb_from4to6(struct sk_buff * old_skb,
	...
	ip6->payload_len = htons(l4len+pay_len);
	...
	switch (ip6->nexthdr) {
		case IPPROTO_UDP:
			...
			//&(l4header.th->source),
			        ...
			        true);
			nat64_adjust_checksum_ipv4_to_ipv6(&(l4header.uh->check), ip4, ip6, true);	


//csum_inv_substract(&l4header.uh->check, (__be16 *)&(outgoing->dst.u3.in6), ((__be16 *)&(outgoing->dst.u3.in6)) + 4);
//		csum_inv_add(&l4header.uh->check, (__be16 *)&ip6->saddr, ((__be16 *)&ip6->saddr) + 16);


pr_debug("NAT64: udp paylen %d", ntohs(l4header.uh->len) );

pr_debug("NAT64: udp tot paylen %d", htons(l4header.uh->len));
			break;
		case IPPROTO_TCP:
			l4header.th = (struct tcphdr *) (ip6 + 1);
...
static struct sk_buff * nat64_get_skb(u_int8_t l3protocol, u_int8_t l4protocol,
	...
	u_int32_t pay_len = skb->len - skb->data_len;
	u_int32_t packet_len, l4hdrlen, l3hdrlen, l2hdrlen;
